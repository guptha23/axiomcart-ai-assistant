"""
data.py — Static product catalog, order database, and support policies.

╔══════════════════════════════════════════════════════════════════╗
║  STAGE 1 CONCEPT: The Agent's World                              ║
║                                                                  ║
║  An agent is only as useful as the data it can access.           ║
║  Here we define three datasets:                                  ║
║                                                                  ║
║    PRODUCT_CATALOG  → fed into a vector store (Stage 2 / RAG)   ║
║    ORDER_DATABASE   → looked up by the support agent (Stage 2+) ║
║    SUPPORT_POLICIES → injected into the support agent's prompt  ║
║                                                                  ║
║  In production these would live in a real database.              ║
║  For the course they are plain Python dicts so you can see       ║
║  every field without navigating a schema.                        ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# ═══════════════════════════════════════════════════════════════════
#  PRODUCT CATALOG
#  This list is embedded into a vector store in rag.py.
#  Each dict becomes one Document; the LLM can search it by meaning.
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
#  ORDER DATABASE
#  Keyed by order ID.  The support agent looks up orders here.
#  A real system would query a database; we use a dict for simplicity.
# ═══════════════════════════════════════════════════════════════════
...
# Filled at runtime by escalate_to_human — acts as an in-memory ticket queue
ESCALATION_QUEUE: list[dict] = []


# ═══════════════════════════════════════════════════════════════════
#  SUPPORT POLICIES
#  Injected verbatim into the support agent's system prompt.
#  Changing policies = changing one string here, not rewriting code.
# ═══════════════════════════════════════════════════════════════════
